...
# DATASETS=("kiba")
DATASETS=("davis")
RUNNING_SETS=("novel-drug" "novel-prot" "novel-pair")
# RUNNING_SETS=("warm")
FOLDS=(0 1 2 3 4)
...
for LEARNING_RATE in 1e-4 5e-5; do
    for BATCH_SIZE in 128 256 64; do

        # Skip the specific combination: LR=1e-4 and BS=64
        if [[ "$LEARNING_RATE" == "1e-4" && "$BATCH_SIZE" == "64" ]]; then
            continue
        fi  
