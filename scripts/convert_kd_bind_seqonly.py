"""
Convert Kd_bind.tsv to DTA training format
...
def convert_kd_bind_seqfilter(input_file, output_file, min_seq_len=50, max_seq_len=1022):
    """
    Convert Kd_bind.tsv to DTA format with sequence length filtering only
    
    Args:
        input_file: Path to Kd_bind.tsv
    ...
    print(f"{'='*80}")
    print("Kd_bind.tsv to DTA Converter (Sequence Length Filter Only)")
    print(f"{'='*80}")
    ...
    print(f"   python code_prepareEmb/get_pretrain.py --dataset kd_bind")
    print(f"\n2. Train model:")
    print(f"   python code/train_full.py --dataset kd_bind --epochs 1000")
    print(f"{'='*80}\n")
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description='Convert Kd_bind.tsv to DTA format (sequence filter only)')
    parser.add_argument('--input', type=str, 
                       default='./data/Kd_bind.tsv',
                       help='Input Kd_bind.tsv file')
    parser.add_argument('--output', type=str,
                       default='./data/kd_bind.txt',
