...
def auto_adjust_max_len(hp, drug_df, prot_df, mol2vec_dict, protvec_dict):
    """
    Automatically adjust max_len hyperparameters based on actual data dimensions.
    
    Args:
        hp: HyperParameter object
        drug_df: Drug dataframe
        prot_df: Protein dataframe
        mol2vec_dict: Dictionary containing drug embeddings
        protvec_dict: Dictionary containing protein embeddings
    """
    print("\n🔍 Analyzing data to auto-adjust max_len hyperparameters...")
    
    # Calculate max drug embedding length
    drug_max_lengths = []
    for drug_id in drug_df['drug_id'].astype(str):
        if drug_id in mol2vec_dict["mat_dict"]:
            drug_mat = mol2vec_dict["mat_dict"][drug_id]
            drug_max_lengths.append(drug_mat.shape[0])
    
    if drug_max_lengths:
        calculated_drug_max = max(drug_max_lengths)
        original_drug_max = hp.drug_max_len
        hp.drug_max_len = calculated_drug_max
        hp.substructure_max_len = hp.drug_max_len
        print(f"  Drug: Original max_len={original_drug_max}, Calculated max={calculated_drug_max}, Adjusted to={hp.drug_max_len}")
    
    # Calculate max protein embedding length
    prot_max_lengths = []
    for prot_id in prot_df['prot_id'].astype(str):
        if prot_id in protvec_dict["mat_dict"]:
            prot_mat = protvec_dict["mat_dict"][prot_id]
            prot_max_lengths.append(prot_mat.shape[0])
    
    if prot_max_lengths:
        calculated_prot_max = max(prot_max_lengths)
        original_prot_max = hp.prot_max_len
        hp.prot_max_len = calculated_prot_max
        print(f"  Protein: Original max_len={original_prot_max}, Calculated max={calculated_prot_max}, Adjusted to={hp.prot_max_len}")
    
    print(f"  Final hyperparameters: drug_max_len={hp.drug_max_len}, prot_max_len={hp.prot_max_len}, substructure_max_len={hp.substructure_max_len}\n")
    return hp
...
if __name__ == "__main__":
   # Parse command line arguments
    ...
    parser.add_argument('--esmc_model', type=str, default=None, choices=['esmc_300m', 'esmc_600m', 'esmc_6b', 'esm3'],
                        help='ESM-C model variant (esmc_300m, esmc_600m, esmc_6b, esm3). Overrides hyperparameter.py setting')
    ...
    hp = HyperParameter(mol_embed_type=args.mol_embed_type)
    ...
    # ============================================================
    # AUTO-ADJUST HYPERPARAMETERS BASED ON DATA (if enabled)
    # ============================================================
    if args.auto_adjust_max_len:
        hp = auto_adjust_max_len(hp, drug_df, prot_df, mol2vec_dict, protvec_dict)
    else:
        print(f"\n⏭️  Skipping auto max_len adjustment. Using default hyperparameters:")
        print(f"  drug_max_len={hp.drug_max_len}, prot_max_len={hp.prot_max_len}, substructure_max_len={hp.substructure_max_len}")
        print(f"  (Use --auto_adjust_max_len flag to enable automatic adjustment)\n")
