...
# LLMDTA -- Encoder, MoE head -- giu nguyen khong doi so voi train_adamW.py).
# ----------------------------------------------------------------------------
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description='Train LLMDTA_FNet (MoE + FourierCrossMixing) for a specific fold')
    ...
    parser.add_argument('--cross_attention_dropout', type=float, default=None,
                        help='Dropout rate for the FourierCrossMixing block (overrides hyperparameter.py setting)')
    ...
    print(f"=" * 60)
    print(f"Training Fold {fold_i}/{hp.kfold-1}  [model = FNet Fourier Mixing (MoE)]")
    ...
    if use_wandb:
        ...
        wandb.init(
            ...
            name=f"fnet-{hp.dataset}-{hp.running_set}-fold{fold_i}-{exp_name}-b{hp.Batch_size}-lr{hp.Learning_rate}-lbw{hp.load_balance_weight}-noise{hp.moe_noise_std}",
            config=wandb_config,
            tags=[hp.dataset, hp.running_set, exp_name, f'fold{fold_i}', 'fnet'],
            reinit=True
        )
    ...
    model = nn.DataParallel(LLMDTA_FNet(hp, device))
    ...
    model_fromTrain = f'./savemodel/{hp.dataset}-{hp.running_set}-fnet-fold{fold_i}-{timestamp}.pth'
    ...
    log_dir = f"./log/{timestamp}-{hp.dataset}-{hp.running_set}-fnet-fold{fold_i}.csv"
    ...
    print(f"{'='*60}")
    predModel = nn.DataParallel(LLMDTA_FNet(hp, device))
    ...
    fold_result_file = f'./log/Test-{hp.dataset}-{hp.running_set}-fnet-fold{fold_i}-{timestamp}.csv'
